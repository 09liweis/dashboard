...
const CATEGORIES = ['Web Development', 'Cost Optimization', 'Static Hosting', 'Platform Engineering'];
          ...
          <div className="flex flex-1 flex-col justify-between p-6 lg:p-8">
            <div className="space-y-4">
              <div className="flex items-center gap-3 text-sm">
                <time className="font-medium text-slate-900" dateTime={blog.getUrl()}>
            ...
            <div className="mt-6 flex items-center justify-between border-t border-slate-100 pt-4">
              <div className="flex flex-wrap gap-2">
                <span className="rounded-md bg-slate-50 px-2.5 py-1 text-xs font-medium text-slate-600 ring-1 ring-inset ring-slate-200/50">
                  Web Dev
                </span>
                <span className="rounded-md bg-slate-50 px-2.5 py-1 text-xs font-medium text-slate-600 ring-1 ring-inset ring-slate-200/50">
                  Performance
    ...
    <article className="group relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white transition-all duration-300 hover:border-slate-300 hover:shadow-lg hover:shadow-slate-200/40">
      <Link href={`/blogs/${blog.getUrl()}`} className="block p-6 sm:p-7">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400">{blog.getReadingTime()}</span>
        </div>
        ...
        <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
          <time className="text-xs font-medium text-slate-400" dateTime={blog.getUrl()}>
            {blog.getFormattedDate()}
          </time>
