    ...
    options: [
      {
        ...
        description: "1–3 pages, no CMS, no user accounts",
        price: 500,
      },
      {
        id: "business",
        label: "Small Business Site",
        description: "5–10 pages with contact form and basic SEO",
        price: 1500,
      },
      {
        id: "portfolio",
        label: "Portfolio Site",
        description: "Showcase work with a blog or case studies",
        price: 1200,
      },
      {
        id: "webapp",
        label: "Web Application",
        description: "User accounts, dashboards, and dynamic data",
        price: 4000,
      },
      {
        id: "ecommerce",
...
export default function Calculator() {
  ...
  const { total, breakdown } = useMemo(() => {
    ...
    return { total: baseTotal + addonTotal, breakdown };
  }, [selections, addons]);
    ...
    <div className="min-h-screen bg-white">
      <section className="border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-6 pt-14 pb-12 md:pt-20 md:pb-16">
          <div>
            <p className="text-sm font-semibold uppercase tracking-widest text-emerald-600 mb-3">Cost Estimator</p>
            <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-gray-900 mb-4">How Much Will My Website Cost?</h1>
            <p className="text-lg text-gray-500 leading-relaxed max-w-2xl">Answer a few questions about your project and get an instant, itemized estimate. No email required.</p>
          </div>
          <div className="mt-8">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-600">{completedCategories} of {CATEGORIES.length} sections answered</span>
              <span className="text-sm font-semibold text-gray-900">{progress}%</span>
            </div>
            <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
              <div style={{ width: `${progress}%`, transition: "width 0.4s" }} className="h-full bg-gray-900 rounded-full" />
            </div>
          </div>
        </div>
      </section>
              ...
              <section key={cat.id}>
                <div className="mb-4">
                  ...
                  <p className="text-sm text-gray-500 mt-0.5">
                    {cat.subtitle}
                    {cat.multiple && <span className="ml-2 px-2 py-0.5 bg-gray-100 text-gray-500 text-xs rounded-full font-medium">Select all that apply</span>}
                  </p>
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={`text-left p-4 rounded-xl border-2 transition-all duration-150 ${
                          isSelected ? "border-gray-900 bg-gray-900 text-white" : "border-gray-200 bg-white text-gray-900 hover:border-gray-400"
                        }`}
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1">
                            <p className={`font-semibold text-sm ${isSelected ? "text-white" : "text-gray-900"}`}>{opt.label}</p>
                            <p className={`text-xs mt-1 leading-relaxed ${isSelected ? "text-gray-300" : "text-gray-500"}`}>{opt.description}</p>
                          </div>
            ...
            <section>
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-500 mt-0.5">
                  ...
                  <span className="ml-1 px-2 py-0.5 bg-gray-100 text-gray-500 text-xs rounded-full font-medium">Select all that apply</span>
                </p>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                {ADDONS.map((addon) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1">
                          <p className={`font-semibold text-sm ${isSelected ? "text-white" : "text-gray-900"}`}>{addon.label}</p>
                          <p className={`text-xs mt-1 leading-relaxed ${isSelected ? "text-gray-300" : "text-gray-500"}`}>{addon.description}</p>
                        </div>
                        <span className={`text-xs font-bold whitespace-nowrap pt-0.5 ${isSelected ? "text-emerald-300" : "text-gray-400"}`}>+{formatPrice(addon.price)}</span>
                      </div>
                      {isSelected && (
                        <div className="mt-2 flex justify-end">
                          <svg className="w-4 h-4 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="lg:sticky lg:top-6">
            <div className="rounded-2xl border border-gray-200 bg-white shadow-lg overflow-hidden">
              <div className="bg-gray-900 px-6 py-5">
                <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-1">Estimated Project Cost</p>
                <p className="text-4xl font-extrabold text-white">{formatPrice(total)}</p>
                {total === 0 && <p className="text-xs text-gray-500 mt-1">Select options above to build your estimate</p>}
              </div>
              <div className="px-6 py-5">
                {total > 0 && (
                  <div className="mb-4">
                    <button type="button" onClick={() => setShowBreakdown((v) => !v)} className="flex items-center gap-2 text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors w-full">
                ...
                <div className="space-y-3">
                  <a href="mailto:weisen.li@hotmail.com" className="block w-full text-center px-5 py-3 bg-gray-900 text-white rounded-xl font-semibold hover:bg-gray-800 transition-colors text-sm">Get a Free Quote</a>
                  <Link href="/faq"><span className="block w-full text-center px-5 py-3 bg-gray-100 text-gray-700 rounded-xl font-semibold hover:bg-gray-200 transition-colors text-sm cursor-pointer">Read the FAQ</span></Link>
                </div>
                <p className="text-xs text-gray-400 text-center mt-4 leading-relaxed">Estimates are approximate. Final pricing depends on exact requirements and a brief discovery call.</p>
              </div>
            </div>
            <div className="mt-5 rounded-xl border border-gray-100 bg-gray-50 p-5">
              <p className="text-xs font-semibold uppercase tracking-widest text-gray-500 mb-3">Typical Ranges</p>
